    ...
    changes: function (obj) {
        ...
        parser.on('opentag', (node) => {
            ...
            if (depth === 2) {
                ...
                const minute = node.attributes.timestamp.slice(0, 16) + ':00Z';
                if (!stats[minute]) stats[minute] = {'_overall': {}};

                const user = node.attributes.user.split(',').join('.');
                if (!stats[minute][user]) stats[minute][user] = {};

                if (stats[minute][user][action + '_' + node.name]) {
                    stats[minute][user][action + '_' + node.name] += 1;
                } else {
                    stats[minute][user][action + '_' + node.name] = 1;
                }

                if (stats[minute]['_overall'][action + '_' + node.name]) {
                    stats[minute]['_overall'][action + '_' + node.name] += 1;
                } else {
                    stats[minute]['_overall'][action + '_' + node.name] = 1;
                }
            }
