    ...
    changes: function (obj) {
        ...
        parser.on('opentag', (node) => {
            ...
            if (depth === 2) {
                ...
                if (stats[minute][user][action + node.name]) {
                    stats[minute][user][action + node.name] += 1;
                } else {
                    stats[minute][user][action + node.name] = 1;
                }

                if (stats[minute]['_overall'][action + node.name]) {
                    stats[minute]['_overall'][action + node.name] += 1;
                } else {
                    stats[minute]['_overall'][action + node.name] = 1;
                }
            }
